	...
	return SignalProducer {
		...
		.flatMap(.merge) { (remoteURL: GitURL) -> SignalProducer<(ProjectEvent?, URL), CarthageError> in
			...
			.flatMap(.merge) { isRepository -> SignalProducer<(ProjectEvent?, URL), CarthageError> in
					...
					.logEvents(identifier: "cloneOrFetch", events: [.starting, .completed], logger: logFetchOperation(for: remoteURL))
					.replayLazily(upTo: 0)
...
private func logFetchOperation(for gitURL: GitURL) -> EventLogger {
	return { identifier, event, _, _, _ in
		print("[\(identifier) \(gitURL)] \(event)")
	}
}
